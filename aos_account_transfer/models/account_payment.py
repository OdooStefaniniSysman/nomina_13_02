...
#     register_date = fields.Date(string='Register Date', required=False, copy=False)
    advance_type = fields.Selection([('invoice', 'Conciliar con la factura'), 
                                     ('advance', 'Pago Inicial'), 
                                     ('advance_emp', 'Anticipo a Empleado'),
                                     ('receivable_emp','Abono de Empleado')], default='invoice', string='Tipo de Anticipo')
    is_force_curr = fields.Boolean('Tasa Negociada')
    force_rate = fields.Monetary('TRM del Documento')
    register_date = fields.Date(string='Fecha de Registro',  default=fields.Date.context_today, required=False, copy=False)
    state = fields.Selection(selection_add=[('confirm', 'Confirmado'),('receipt','Recibido')])
    #================make charge transfer=======================================
    amount_charges = fields.Monetary(string='Gastos del Movimiento', required=False)
    #charge_account_id = fields.Many2one('account.account', string='Account Adm', domain=[('user_type_id.name','=','601.00 Expenses')])
    charge_account_id = fields.Many2one('account.account', string='Cuenta de Gastos', domain=[])
    payment_adm = fields.Selection([
            ('cash','Caja'),
            ('free_transfer','Transferencia sin Gastos Bancarios'),
            ('transfer','Transferencia'),
            #('check','Check/Giro'),
            #('letter','Letter Credit'),
            ('cc','Tarjeta de Crédito'),
            ('dc','Tarjeta de Débito'),
            ],string='Gastos del Movimiento')
    card_number = fields.Char('Número de Tarjeta', size=128, required=False)
    card_type = fields.Selection([
            ...
            ('citi','CITI Card'),
            ('amex','AMEX'),
            ], string='Tipo de Tarjeta', size=128)
