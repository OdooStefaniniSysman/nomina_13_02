...
class SaleOrderLine(models.Model):
    ...
    city_id = fields.Many2one('res.city.zip', string='Ciudad')
    partner_id = fields.Many2one('res.partner', string='Tercero', domain="[('company_id', '=', company_id),('company_id','=',False)]")
    ...
    def product_id_change(self):
        ...
        # if not self.city_id:
        #     self.city_id = self.order_id.city_id
        return result

    def _prepare_invoice_line(self):
        res = super(SaleOrderLine, self)._prepare_invoice_line()
        res['city_id'] = self.city_id.id or False
        return res