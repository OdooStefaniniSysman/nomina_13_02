...
class AccountBankStatementImportSheetParser(models.TransientModel):
    ...
    def _parse_lines(self, mapping, data_file, currency_code):
        ...
        data_list = list(csv_or_xlsx)
        if mapping.is_only_positive:
                x = []
                for l in range(0, len(data_list)):
                    if float(data_list[l][columns['amount_column']]) < 0 or '-' in data_list[l][columns['amount_column']]:
                        x.append(data_list[l])
                for p in x:
                    data_list.pop(data_list.index(p))
        if mapping.is_without_dv and columns.get('partner_name_column', False):
            x = 0
            for l in range(0, len(data_list)):
                # if x > 0:
                print('qwer', l)
                size = len(data_list[l][columns['partner_name_column']])
                data_list[l][columns['partner_name_column']] = data_list[l][columns['partner_name_column']][:size-1]
                # x += 1
        csv_or_xlsx = data_list
        # csv_or_xlsx = writer(StringIO(data_file.decode(mapping.file_encoding or "utf-8")), **csv_options)
        # csv_or_xlsx.writerows(data_list)
        # with open("salida.csv", "wb") as fichero:
        #     csv_or_xlsx = writer(fichero)
        #     csv_or_xlsx.writerows(data_list)
