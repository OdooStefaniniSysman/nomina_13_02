...
class CrmLead(models.Model):
    ...
    benefit_center=fields.Many2one('benefit.center', string="Centro de beneficio", tracking=True)
    segment_crm = fields.Many2one('crm.segment', string="Segmento", tracking=True)
    bussinets_units = fields.Many2one('bussinets.units', tracking=True)
    ...
    tender_type = fields.Selection([('PÚBLICA', 'PÚBLICA'),
                                      ...
                                      ('DIRECTA', 'DIRECTA'),
                                      ('PRIVADA', 'PRIVADA'),
                                      ('SUBASTA', 'LICITACIÓN POR SUBASTA')], string="Tipo de Licitación")
    ...
    tender_budget = fields.Float('Presupuesto de licitación', tracking=True)
    ...
    approval_director = fields.Selection([('SI', 'SI'),
                                           ('NO', 'NO')], string='Aprobación de director Comercial', tracking=True)
    ...
    final_contract_value = fields.Float('Valor final del contrato', tracking=True)
    number_rup = fields.Integer('Número de consecutivo en el RUP', tracking=True)
    ...
    tag_ids = fields.Many2many('crm.lead.tag', 'crm_lead_tag_rel', 'lead_id', 'tag_id', string='Tags', help="Classify and analyze your lead/opportunity categories like: Training, Service",  required=True)
    observations_contract=fields.Char('Observaciones')
    fields_update=fields.Char('Campos actualizados')
    ...
    def force_one_selection(self):
        if self.tag_ids and len(self.tag_ids) > 1:
           raise ValidationError("No se pueden registrar más categorías")
    ...
    def stage_prop(self):
        for record in self:
            record.check_stage = True if record.type == 'opportunity' and record.stage_id.id != 1 else False
    ...
    def _check_union(self):
        for record in self:
            if record.temporary_union == 'SI' and not record.member_union_id.name and not record.member_union_id.participation and not record.member_union_id.funtions:
    ...
    ####################################################Adicionar segmento y unidad de negocio segun tipo de beneficio
    @api.onchange('benefit_center')
    def _benefit_center_units(self):
        for record in self:
            if record.benefit_center:
                record.segment_crm = record.benefit_center.segment_crm
                record.bussinets_units = record.benefit_center.bussinets_units 
            else:
                record.segment_crm = False
                record.segment_crm = False

    @api.onchange('partner_id')
    def _onchange_state(self):
        self._onchange_compute_probability(optional_field_name='partner_id')
    ...
    def write(self, values):
        for record in self:
            if record.tag_ids[0].name == 'LICITACIÓN':
                record.send_email_tag()
                
        if values.get('stage_id') and self.stage_id.id == 1 and self.type == 'opportunity':
            if self.comercial_approval!='SI' :
                raise ValidationError("Se requiere la aprobación de Gerencia Comercial en el apartado Costos")
            if self.cost_total<=0 :
                raise ValidationError("El valor del costo total no puede ser menor o igual que 0.00")
            if self.quantity<=0:
                raise ValidationError("El valor de la cantidad no puede ser menor o igual que 0.00")
            if self.unit_price<=0:
                raise ValidationError("El valor del precio unitario no puede ser menor o igual que 0.00")
            if self.price_n_iva<=0:
                raise ValidationError("El valor del precio sin IVA no puede ser menor o igual que 0.00")
            if self.contribution_percentage<=0:
                raise ValidationError("El valor del porcentaje de contribución no puede ser menor o igual que 0.00")
            if not self.cost_id and not values.get('cost_id'):
                raise ValidationError("Es necesario registrar la tabla de Costos cuando se ha superado la etapa Propuesta")
            
        res = super(CrmLead, self).write(values)


        if self.stage_id.id == 4 and self.partner_id.customer_type == 'PROSPECTO' and self.partner_id.country_id.id == 49 and not self.partner_id.rut:
                raise ValidationError("Se debe adjuntar el RUT antes de pasar al estado CONTRATO")

        if self.stage_id.id != 1 and self.comercial_approval!='SI' :
            raise ValidationError("Se requiere la aprobación de Gerencia Comercial en el apartado Costos")
        if self.stage_id.id != 1 and self.cost_total<=0 :
            raise ValidationError("El valor del costo total no puede ser menor o igual que 0.00")
        if self.stage_id.id != 1 and self.quantity<=0:
            raise ValidationError("El valor de la cantidad no puede ser menor o igual que 0.00")
        if self.stage_id.id != 1 and self.unit_price<=0:
            raise ValidationError("El valor del precio unitario no puede ser menor o igual que 0.00")
        if self.stage_id.id != 1 and self.price_n_iva<=0:
            raise ValidationError("El valor del precio sin IVA no puede ser menor o igual que 0.00")
        if self.stage_id.id != 1 and self.contribution_percentage<=0:
            raise ValidationError("El valor del porcentaje de contribución no puede ser menor o igual que 0.00")
        if self.stage_id.id != 1 and not self.cost_id and self.type == 'opportunity':
            raise ValidationError("Es necesario registrar la tabla de Costos cuando se ha superado la etapa Propuesta")
        return res
##################################################################
    def send_email_tag(self):
        for record in self:
            template = self.env.ref('contacs_thomas.cambios_campos_en_licitacion')
            if template:
