    ...
    def modify(self):
        if self.records_ids and not self.is_model_change:
            ...
            for asset in self.records_ids:
                modify_obj = self.env['asset.modify'].create({'asset_id': asset.id, 'date': self.date, 'name': 'reanudacion masiva'})
                modify_obj.modify()
        else:
            if self.records_ids and self.is_model_change:
                for records in self.records_ids:
                    records.modify_model('BOTH',self.model_id)
                    childs = self.env['account.asset'].search([('asset_parent_id','=',records.id)])
                    if childs:
                        for child in childs:
                            child.modify_model('BOTH', self.model_id)
