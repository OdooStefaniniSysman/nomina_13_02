    ...
    def create(self, vals):
        if 'asset_id' in vals:
            ...
            if 'method_number' not in vals:
                vals.update({'method_number': len(asset.depreciation_move_ids.filtered(lambda move: move.state != 'posted' and move.asset_clasification=='FISCAL')) or (asset.method_number - len(asset.depreciation_move_ids.filtered(lambda move: move.state == 'posted' and move.asset_clasification=='FISCAL'))) or 1})
