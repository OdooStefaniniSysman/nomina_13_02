...
class AccountReconciliation(models.AbstractModel):
    ...
    def _domain_move_lines_for_reconciliation(self, st_line, aml_accounts, partner_id,excluded_ids=None, search_str=False, mode='rp'):
        domain = super()._domain_move_lines_for_reconciliation(
                st_line, aml_accounts, partner_id,excluded_ids=excluded_ids, search_str=search_str, mode='rp')
        domain = expression.AND([domain, [("account_id.exclude_bank_reconcile", "!=", True)]])
        ...
        if account_reconciliation_ids:
            domain = expression.AND([domain, [("account_id", "not in", account_reconciliation_ids.ids)]])
        return domain
