...
class HrDeductionRF(models.Model):
    ...
    @api.onchange('max_uvt')
    def onchange_max_uvt(self):
        value = 0.0
        active_uvt = self.env['hr.value.uvt.rf'].search([
            ('status', '=', True)])
        if active_uvt:
            self.max_value = active_uvt.value * self.max_uvt
        return {'max_value': value}

    def change_max_value_action(self):
        """Send to Employee."""
        active_uvt = self.env['hr.value.uvt.rf'].search([
            ('status', '=', True)])
        for rec in self:
            if active_uvt:
                rec.write({'max_value': active_uvt.value * rec.max_uvt})
...
class HrRetentionMethod(models.Model):
    ...
    name = fields.Char(required=True)
    description = fields.Text()
    non_resident = fields.Boolean(string='Non-resident')
...
class HrEmployee(models.Model):
    ...
    payslip_deductions_ids = fields.One2many(
        'hr.payslip.deductions.rf', 'employee_id',
        string='Deductions', help="The list of deductions for employee")
    deductions_accumulate_ids = fields.One2many(
        'hr.deduction.accumulate.rf', 'employee_id',
        string='Accumulate Deductions', help="The accumulate of deductions for year")
    is_dependientes = fields.Boolean(
        'Dependientes', tracking=True)
...
class HrUVTValueRF(models.Model):
    ...
    percentage_max_deduction = fields.Float(
        ...
        track_visibility='always')
    non_resident_percentage = fields.Float(
        string="Non-resident Percentage",
        track_visibility='always')
    ...
    rule_ids = fields.One2many(
        'hr.value.uvt.input.not.rf', 'hr_uvtvalue_id', string='Rules Input Not RF',
        shelp="The list of range UVT")
...
class HrSalaryRule(models.Model):
    ...
    code_sap = fields.Char(string="Code SAP")
...
class HrPayslipDeductionsRF(models.Model):
    ...
    employee_id = fields.Many2one(
        related="hr_payslip_id.employee_id", store="True")
    description = fields.Text()
...
class HrPayslip(models.Model):
    ...
    total_retention_income_cont = fields.Float(
        ...
        store=True)
    value_beneficity_trib = fields.Float(
        string="Value Beneficity Trib",
        compute='_compute_value_beneficity_trib',
        store=True)
    method_renting_calculate = fields.Many2one(
        'hr.retention.method.rf', string="Retention Method calculate")
    percentage_renting_calculate = fields.Float(
        string="Percentage Renting Calculate", digits=(32, 2))
        ...
        for rec in self:
            ...
            if rec.state in ('draft', 'verify'):
                for line in rec.ps_renting_additional_ids:
                    if line.value_final:
                        result += line.value_final
                type_deduction = self.env['hr.deduction.type.rf'].search([
                    ('sequence', '=', 3)], limit=1)
                if type_deduction:
                    limit_value = (rec.inputs_rent *
                                   type_deduction.limit_percentage) / 100
                if limit_value < result:
                    result = limit_value
            rec.sum_rent_add = result
    ...
    def _compute_total_renting_excempt(self):
        ...
        for rec in self:
            ...
            accumulate_id = self.env['hr.deduction.accumulate.rf'].search([('year', '=', rec.date_to.year),
    ...
    def _compute_subtotal_3(self):
        result = 0
        for reg in self:
            reg.subtotal_3 = reg.subtotal_2 - reg.sum_rent_add
        return result
    ...
    def _compute_calculate_renting_exempt_cont(self):
        ...
        for reg in self:
            result = \
                reg.subtotal_3_cont * active_uvt.percentage_renting_exempt / 100
            if result > active_uvt.max_value_renting_exempt:
                result = active_uvt.max_value_renting_exempt
    ...
    @api.depends('ps_exempt_income_ids', 'subtotal_1',
                 'subtotal_2', 'subtotal_3_cont', 'calculate_renting_exempt_cont')
    def _compute_subtotal_4_cont(self):
        result = 0
        for reg in self:
            reg.subtotal_4_cont = reg.subtotal_3_cont - reg.calculate_renting_exempt_cont
        return result
    ...
    @api.depends('subtotal_1', 'subtotal_2', 'subtotal_3_cont', 'subtotal_4_cont')
        ...
        for rec in self:
            ...
            if rec.state in ('draft', 'verify'):
                if rec.subtotal_1 and rec.total_deductions_renting:
                    result = rec.subtotal_1 - \
                             rec.total_deductions_renting
                    rec.total_base_retention = result
            rec.total_base_retention = result
    ...
    def _compute_total_retention_income(self):
        ...
        for reg in self:
            ...
            result_days = sum([work_days.number_of_days for work_days
                               in reg.worked_days_line_ids if work_days.name not in list_leave])
            days0 = False
            basic_days = 30
            if result_days > 30:
            ...
            if result_days == 0:
                days0 = True
                result_days = 30
            # Generate process without fraction days
            basic_days = 30
            result_days = 30
                ...
                if reg.contract_completion_id:
                    if reg.employee_id.retention_method_id.name == '1':
                        for uvtitem in uvtvalue_id.range_retention_ids:
                            if reg.income_represented_uvt < uvtitem.value_uvt_max and reg.income_represented_uvt > uvtitem.uvt_discount:
                                reg.percentage_renting_calculate = uvtitem.marginal_rate
                                result = 0
                                result = ((reg.income_represented_uvt - uvtitem.uvt_discount) * uvtvalue_id.value) * \
                                         uvtitem.marginal_rate / 100 + \
                                         (uvtitem.uvt_additional * uvtvalue_id.value)
                        if result_days < 0:
                            value_real_ret = (
                                                     round(result, -3) / basic_days) * (result_days + 30)
                            value_month_before = reg.employee_id.get_value_line_payslip_before(
                            ...
                            result_days = sum([work_days.number_of_days for work_days
                                               in payslip_ids.worked_days_line_ids if work_days.name not in list_leave])
                            value_real_ret = (
                        ...
                        result = reg.base_income_retention * reg.employee_id.withholding_2 / 100
                        reg.percentage_renting_calculate = reg.employee_id.withholding_2
                    ...
                    if reg.employee_id.retention_method_id.name == '1':
                        ...
                        reg.total_retention_income = round(
                            ((result / basic_days) * result_days), -3)
                    elif reg.employee_id.retention_method_id.non_resident:
                        result = reg.inputs_rent * uvtvalue_id.non_resident_percentage / 100
                        reg.percentage_renting_calculate = uvtvalue_id.non_resident_percentage
                        reg.total_retention_income = round(
                            ((result / basic_days) * result_days), -3)
                    else:
                        reg.total_retention_income = 0
    ...
    def _compute_total_retention_income_cont(self):
        ...
        for reg in self:
            ...
            if uvtvalue_id:
                if reg.employee_id.retention_method_id.name == '1':
                    for uvtitem in uvtvalue_id.range_retention_ids:
                        if reg.income_represented_uvt_cont < uvtitem.value_uvt_max \
                                and reg.income_represented_uvt_cont > uvtitem.uvt_discount:
                            reg.percentage_renting_calculate_cont = uvtitem.marginal_rate
                            result = 0
                            # result = (reg.base_income_retention - (uvtvalue_id.value* uvtitem.uvt_discount)) * uvtitem.marginal_rate/100 + (uvtitem.uvt_additional * uvtvalue_id.value)
                            result = ((reg.income_represented_uvt_cont - uvtitem.uvt_discount) * uvtvalue_id.value) \
                                     * uvtitem.marginal_rate / 100 + (uvtitem.uvt_additional * uvtvalue_id.value)
                    reg.total_retention_income_cont = (
                                                              round(result, -3) / basic_days) * result_days
                elif reg.employee_id.retention_method_id.name == '2':
                    result = reg.base_income_retention_cont * reg.employee_id.withholding_2 / 100
                    reg.percentage_renting_calculate_cont = reg.employee_id.withholding_2
                    reg.total_retention_income_cont = (
                                                              round(result, -3) / basic_days) * result_days
                elif reg.employee_id.retention_method_id.non_resident:
                    result = reg.inputs_rent * uvtvalue_id.non_resident_percentage / 100
                    reg.percentage_renting_calculate = uvtvalue_id.non_resident_percentage
                    reg.total_retention_income_cont = round(
                        ((result / basic_days) * result_days), -3)
                else:
    ...
    @api.depends('total_retention_income', 'total_retention_income_cont')
    def _compute_value_beneficity_trib(self):
        result = 0
        for reg in self:
            reg.value_beneficity_trib = reg.total_retention_income_cont - reg.total_retention_income
        return result
    ...
    def compute_sheet_rf(self):
        ...
        for s_id in self:
            ...
            list_leave = (list_leaves)
            result_days = sum([work_days.number_of_days for work_days
                               in s_id.worked_days_line_ids if work_days.name not in list_leave])
            ...
            if s_id.contract_completion_id and mrt == '1' and resultdays == 0:
                ...
                for input_id in uvt_active.irf_rule_ids:
                    for ps_line_id in s_id.line_ids.filtered(lambda x:
                        ...
                        vals = {'name': input_id.name,
                                ...
                                'value_final': ps_line_id.salary_rule_id.projection_exempt and
                                               ps_line_id.total or
                ...
                for input_id in uvt_active.irf_rule_ids:
                    for ps_line_id in s_id.line_ids.filtered(lambda x:
                        ...
                        if ps_line_id.total > 0 and result_days < 0 and \
                                not ps_line_id.salary_rule_id.projection_exempt:
                            auxsign = -1
                        vals = {'name': input_id.name,
                                'rule_id': input_id.rule_id.id,
                                'sequence': input_id.sequence,
                                'value_final': (ps_line_id.salary_rule_id.projection_exempt and
                                                ps_line_id.total or
                                                (ps_line_id.total / result_days) * basic_days) * auxsign,
                                'hr_payslip_id': s_id.id,
                                }
                        s_id.env['hr.payslip.input.rf'].create(vals)
                if s_id.payslip_type_id and s_id.payslip_type_id.control_payslip:
                    payslip_ids = self.env['hr.payslip'].search(
                        [('state', 'in', ['done', 'paid']),
                         ('employee_id', '=', s_id.employee_id.id),
                         ('date_from', '>=', s_id.date_from),
                         ('date_to', '<=', s_id.date_to),
                         ])
                    for payslip in payslip_ids:
                        for input_id in uvt_active.irf_rule_ids:
                            for ps_line_id in payslip.line_ids.filtered(lambda x:
                                                                        input_id.rule_id == x.salary_rule_id and
                                                                        input_id.exempt_rm_id.name != mrt):
                                auxsign = 1
                                if ps_line_id.total > 0 and result_days < 0 and \
                                ...
                                s_id.env['hr.payslip.input.rf'].create(vals)
            if not self.employee_id.retention_method_id.non_resident:
                if s_id.payslip_type_id and s_id.payslip_type_id.control_payslip:
                    payslip_ids = self.env['hr.payslip'].search(
                        [('state', 'in', ['done', 'paid']),
                         ('employee_id', '=', s_id.employee_id.id),
                    ...
                    for payslip in payslip_ids:
                        for input_id in uvt_active.rule_ids:
                            for ps_line_id in payslip.line_ids.filtered(lambda x: input_id.rule_id == x.salary_rule_id):
                                vals = {'name': input_id.name,
                                        'rule_id': input_id.rule_id.id,
                                        'sequence': input_id.sequence,
                                        'value_final': ps_line_id.salary_rule_id.projection_exempt and
                                ...
                                s_id.env['hr.payslip.input.not.rf'].create(vals)
                for input_id in uvt_active.rule_ids:
                    for ps_line_id in s_id.line_ids.filtered(lambda x: input_id.rule_id == x.salary_rule_id):
                        vals = {'name': input_id.name,
                                'rule_id': input_id.rule_id.id,
                                'sequence': input_id.sequence,
                                'value_final': ps_line_id.salary_rule_id.projection_exempt and
                ...
                for ded in s_id.employee_id.deductions_ids:
                    ...
                    if ded.value == 0 and ded.percentage != 0:
                        val_per = s_id.inputs_rent * \
                                  ded.percentage / 100
                        value_reference, value_final = val_per, val_per
                    else:
                    ...
                    if not ded.start_date and \
                            ded.end_date and s_id.date_to >= ded.end_date:
                        add_in_rent = True
                    if ded.start_date and s_id.date_from.month == ded.start_date.month and \
                    ...
                    if add_in_rent:
                        ...
                        if ded.hr_deduction_id.max_uvt > 0:
                            ...
                            if value_final > val_max:
                                value_final = val_max
                        if ded.hr_deduction_id.max_percentage > 0:
                            val_max = s_id.inputs_rent * \
                                      ded.hr_deduction_id.max_percentage / 100
                            if value_final > val_max:
                                value_final = val_max
    ...
    def create(self, vals):
        ...
        if employee_id.retention_method_id.non_resident:
            uvtvalue_id = self.env['hr.value.uvt.rf'].search(
                [('status', '=', True)])
            vals['percentage_renting_calculate'] = uvtvalue_id.non_resident_percentage
        vals[
            'method_renting_calculate'] = employee_id.retention_method_id and employee_id.retention_method_id.id or False
