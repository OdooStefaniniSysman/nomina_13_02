...
class AccountInvoice(models.Model):
	...
	effective_delivery_date = fields.Date(string='Fecha entrega')
	dian_resolution = fields.Char(string="Resolución DIAN",tracking=True)
	sale_journal_type = fields.Selection(string='Tipo diario de venta', related='journal_id.sale_journal_type')
	...
	multi_customer_ids = fields.One2many('account.move.multi.customer', 'move_id', string='Adquirientes', copy=True)
	...
	def validate_amount_credit_note(self):
		self.ensure_one()
		if self.type == 'out_refund' and self.reversed_entry_id:
			invoice_origin = self.reversed_entry_id
			credit_notes = self.env['account.move'].search([('reversed_entry_id','=',invoice_origin.id),('type','=','out_refund'),('state','!=','cancel')])
			if invoice_origin.amount_total < sum(credit_notes.mapped('amount_total')):
				msg = _('valor factura: %s \n\n Id,Nombre,Valor,Estado \n') % invoice_origin.amount_total
				for x in credit_notes:
					msg += '{}, {}, {}, {}\n'.format(x.id, x.name, x.amount_total, x.state)
				raise ValidationError(_('Los montos de las notas crédito realizadas a la factura %s superan el valor de la factura. \n\n %s') % (invoice_origin.name, msg))
	...
	def post(self):
		...
		_logger.info('validatee')
		for record in self:
			record.validate_amount_credit_note()

		res = super(AccountInvoice, self).post()
		for record in self:
			_logger.info(record.type)
			if record.company_id.einvoicing_enabled and record.journal_id.sale_journal_type == 'einv':
	...
	def _get_billing_reference(self):
		billing_reference = {}
		msg1 = ''
		msg2 = _('La nota de crédito no tiene referencia de facturación \n\n')
			...
			if origin_invoice.state in ('open', 'paid', 'posted'):
				for dian_document in origin_invoice.dian_document_lines:
					if dian_document.state == 'done' or dian_document.output_comfiar_status_code in ('ACEPTADO', 'AUTORIZADO'):
						...
						msg1 = _('Estado DIAN de la factura: ') + dian_document.output_comfiar_status_code
	...
	def _get_invoice_lines(self):
		...
		for invoice_line in self.invoice_line_ids.sorted(lambda x: x.sequence):
			...
			if invoice_line.display_type == 'line_note':
				continue
			...
			if self.operation_type == '09':
				nota_ref = invoice_line.ref_comfiar + ', Contrato de servicios AIU por concepto de: ' + self.aiu
			else:
				nota_ref = invoice_line.ref_comfiar

			invoice_lines[count] = {}
			# invoice_lines[count]['DocOrigin'] = invoice_line.ref_comfiar
			invoice_lines[count]['NumberItem'] = invoice_line.item
			...
			if self.company_id.members_ids:
		...
		for move in self:
			...
			for line in move.line_ids:
				for tax in line.tax_ids.flatten_taxes_hierarchy().filtered(lambda t: t.amount == 0.0):
					res.setdefault(tax.tax_group_id.tax_group_type_id.code, {'name': line.tax_line_id.tax_group_id.tax_group_type_id.name, 'base': 0.0, 'amount': 0.0})
					res[tax.tax_group_id.tax_group_type_id.code]['base'] += tax_balance_multiplicator * (line.amount_currency if line.currency_id else line.balance)

			return res
	
	# @api.onchange('date', 'currency_id', 'partner_id')
	# def _onchange_currency(self):
	# 	if self.partner_id and self.partner_id.inv_without_decimal:
	# 		self = self.with_context(currency_rounding=1)
	# 	res = super(AccountInvoice, self)._onchange_currency()

	# 	return res

	def action_print_supplier_voucher(self):
		action = self.env.ref('l10n_co_e_invoicing_comfiar.action_view_supplier_voucher_wizard').read()[0]
	...
	def create(self, vals):
		...
		if res.type == 'out_refund':
			for line in res.invoice_line_ids.filtered(lambda l: l.product_id):
				account_reutn_reco = line.product_id.property_account_return_income_id.id or line.product_id.categ_id.property_account_return_income_categ_id.id or False
				if not account_reutn_reco:
					raise ValidationError(_('The product %s does not have an income reimbursement account. set it in the product or the category.') % line.product_id.display_name)
				line.account_id = account_reutn_reco
		days = res.company_id.effective_delivery_day
		date = res.create_date + timedelta(days=days)
		res.effective_delivery_date = date.astimezone(timezone(self.env.user.tz)).date()
		# res.validate_amount_credit_note()
		return res

	def _recompute_payment_terms_lines(self):
		''' Compute the dynamic payment term lines of the journal entry.'''
		if self.type not in ('out_invoice', 'out_refund') or not (self.is_multi_customer or (self.journal_id and self.journal_id.sale_journal_type == 'reco')):
			return super(AccountInvoice, self)._recompute_payment_terms_lines()
		...
		def _get_payment_terms_account(self, payment_terms_lines):
			...
			'''
			if self.journal_id and self.journal_id.sale_journal_type == 'reco':
				account_recognition_id = self.journal_id.account_recognition_id or False
				if not account_recognition_id:
					raise ValidationError(_('There is no revenue recognition account, set it up in the journal.'))
				if self.is_multi_customer:
					accounts = {}
					for p in self.multi_customer_ids:
						accounts[str(p.partner_id.id)] = account_recognition_id
					return accounts
				else:
					return account_recognition_id
				...
				if self.is_multi_customer:
					accounts = {}
					for p in self.multi_customer_ids:
						for payment_terms_line in payment_terms_lines.filtered(lambda l: l.partner_id.id == p.partner_id.id):
							accounts[str(p.partner_id.id)] = payment_terms_line.account_id
					return accounts
				else:
				...
				if self.is_sale_document(include_receipts=True):
					if self.is_multi_customer:
						accounts = {}
						for partner in self.multi_customer_ids:
							accounts[str(partner.partner_id.id)] = partner.partner_id.property_account_receivable_id
						return accounts
					else:
		...
		def _compute_diff_payment_terms_lines(self, existing_terms_lines, account, to_compute, partner):
			...
			for date_maturity, balance, amount_currency in to_compute:
				...
				if existing_terms_lines_index < len(existing_terms_lines):
					...
					candidate = create_method({
						...
						'account_id': account.id,
						'partner_id': partner.commercial_partner_id.id or self.commercial_partner_id.id,
		...
		if self.is_multi_customer:
			...
			for p in self.multi_customer_ids:
				...
				for to_comp in to_compute:
					to_c.append((to_comp[0], to_comp[1]*p.percent/100, to_comp[2]))
				new_terms_line = _compute_diff_payment_terms_lines(self, existing_terms_lines.filtered(lambda l: l.partner_id.id == p.partner_id.id), account[str(p.partner_id.id)], to_c, p.partner_id)
				new_terms_lines += new_terms_line	
		else:
			new_terms_lines = _compute_diff_payment_terms_lines(self, existing_terms_lines, account, to_compute, False)
	...
	# @api.onchange('journal_id', 'invoice_line_ids', 'line_ids')
	# def _onchange_account_reco(self):
	# 	if self.journal_id and self.journal_id.sale_journal_type == 'reco':
	# 		account_recognition_id = self.journal_id.account_recognition_id.id or False
	# 		if not account_recognition_id:
	# 			raise ValidationError(_('There is no revenue recognition account, set it up in the journal.'))
	# 		else:
	# 			for line in self.line_ids.filtered(lambda l: l.account_id.user_type_id.type in ('receivable', 'payable')):
	# 				line.account_id = account_recognition_id
	...
	def _onchange_journal(self):
		...
		if self.journal_id and self.type in ('out_invoice', 'out_refund'):
			if self.journal_id.sale_journal_type in ('no', 'einv','reco'):
				line_to_remove = self.env['account.move.line']
				for line in self.line_ids.filtered(lambda l: l.account_id.user_type_id.type in ('receivable',)):
					line_to_remove += line
				if line_to_remove:
					self.line_ids -= line_to_remove
			self._recompute_dynamic_lines()
		return res
