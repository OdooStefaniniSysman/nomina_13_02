...
class AccountInvoiceDianDocument(models.Model):
    ...
    dian_line_ids = fields.One2many(
        comodel_name='dian.document.line',
    ...
    last_state_date = fields.Datetime(string='Fecha ultimo envío con hora', compute='_get_last_state_date')
    customer = fields.Char(string="Cliente", related="invoice_id.partner_id.name")
    nit = fields.Char(string="Nit", related="invoice_id.partner_id.vat")

    @api.depends('dian_line_ids')
    def _get_last_state_date(self):
        for record in self:
            if record.dian_line_ids:
                record.last_state_date = record.dian_line_ids.sorted(lambda x: x.id)[-1].create_date
            else:
                record.last_state_date = False
    ...
    def _get_xml_values(self, ClTec):
        ...
        ActualDeliveryDate = self.invoice_id.effective_delivery_date
        ...
        OrderReferenceID = False
        OrderReferenceIssueDate = False
        if self.invoice_id.ref1_comfiar or self.invoice_id.purchase_order_date:
            if not self.invoice_id.ref1_comfiar:
                raise ValidationError(_('Hace falta la orden de referencia'))
            if not self.invoice_id.purchase_order_date:
                raise ValidationError(_('Hace falta la fecha de la orden de compra'))
            OrderReferenceID = self.invoice_id.ref1_comfiar
            OrderReferenceIssueDate = self.invoice_id.purchase_order_date
        
        AddDocRefID1 = self.invoice_id.additional_document_reference or False
        AddDocRefID2 = self.invoice_id.additional_document or False
        ...
        # else:
        #     for email in self.company_id.einvoicing_email.split(','):
        #         receptores = dict(receptores, **{email.replace(' ', ''): {'Name': self.invoice_id.partner_id.name,
        #                                                                 'Telephone': '12345',
        #                                                                 }})
        ...
        if Uts:
            ...
            if Uts_rest > 0:
                while ut_c <= Uts_rest:
                    ...
                    ut_c += 1
        # _logger.info('\n\n ****** ******')
        # _logger.info(Uts_extra)
        # _logger.info('\n\n ****** ******')
        self.invoice_id.dian_resolution = str(active_dian_resolution['resolution_number']) or ''
        ...
        return {
            ...
            'OrderReferenceIssueDate': OrderReferenceIssueDate,
            'AddDocRefID1': AddDocRefID1,
            'AddDocRefID2': AddDocRefID2,
            'PartyLegalEntity': self.get_partylegalentity(),
            'FormatoID': self.company_id.formatoId,}
    ...
    def get_partylegalentity(self):
        ...
        partner_ids = self.invoice_id.multi_customer_ids.mapped('partner_id') if self.invoice_id.is_multi_customer else self.invoice_id.partner_id

        data = {}
        for partner_id in partner_ids:
            if partner_id.document_type_id:
                document_type_code = partner_id.document_type_id.code
                name = partner_id.name
                identification_document = partner_id.identification_document or False
                check_digit = ''

                if document_type_code == '31' and not partner_id.check_digit:
                    raise UserError(msg5 % partner_id.name)
                if document_type_code == '31':
                    check_digit = partner_id.check_digit

                #Punto 13.2.1. del anexo técnico version 1.8
                if document_type_code not in ('11', '12', '13', '21', '22', '31', '41', '42', '50', '91'):
                    if partner_id.person_type == '1':
                        raise UserError(msg6 % partner_id.name)
                    else:
                        name = 'usuario final'
            ...
            if not identification_document:
                raise UserError(msg7 % partner_id.name)
            
            data[str(partner_id.id)] = {
                'Name': name,
                'CompanyIDschemeName': document_type_code,
                'CompanyIDschemeID': check_digit,
    ...
    def _get_invoice_values(self):
        ...
        if self.invoice_id.partner_id.inv_without_decimal:
            xml_values = self.rounding_numbers(xml_values, 0)
            file_dict = open('xml_without_decimal.py', 'wb')
            file_dict.write(str(xml_values).encode('utf-8'))
        else:
            file_dict = open('xml_with_decimal.py', 'wb')
            file_dict.write(str(xml_values).encode('utf-8'))
        return xml_values
    ...
    def _get_xml_file(self):
        ...
        self.create_historical_dian(end_state='0')
    ...
    def create_historical_dian(self, end_state=False):
        self.ensure_one()
        lines = self.dian_line_ids.sorted(lambda line: line.id)
        start_state = ''
        if lines:
            start_state = lines[-1].end_state
        vals = {
            'dian_document_id': self.id,
            ...
            'end_state': '',
            'state_comfiar': self.output_comfiar_status_code or '',
            'description': self.output_comfiar_response or '',
        }
        if self.state == 'draft' or end_state == '0':
            end_state = '0'
            vals.update(vals, **{
                'end_state': '0',
                'state_comfiar': _('XML Generated'),
            })
        elif self.output_comfiar_status_code == 'AUTORIZADO':
            end_state = '2'
        elif self.output_comfiar_status_code == 'ACEPTADO':
            end_state = '1'
        elif self.output_comfiar_status_code in ('TransaccionSinTerminar', 'AProcesar', 'CargandoComprobantes', 'ProcesandoOrganismoFiscal', 'GuardandoResultado', 'ProcesandoAFIP'):
            end_state = '3'
        elif self.output_comfiar_status_code in ():
            end_state = '10'
        elif self.output_comfiar_status_code in ('ResponseError', 'ERROR'):
            end_state = '11'
        elif self.output_comfiar_status_code == 'RECHAZADO':
            end_state = '12'
        else:
            vals.update(vals, **{
                'state_comfiar': _('Other'),
            })
        vals.update(vals, **{'end_state': end_state,})

        self.env['dian.document.line'].sudo().create(vals)
    
    def rounding_numbers(self, xml_values, rounding=2):
        self.ensure_one()
        for k, v in xml_values.items():
            if k in ('LineExtensionAmount', 'TaxExclusiveAmount', 'TaxInclusiveAmount', 'PayableAmount'):
                v = '{:.2f}'.format(round(float(v), rounding))
                xml_values[k] = v
                print('\n\n**********CABECERA***********\n\n')
            elif k in ('TaxesTotal', 'WithholdingTaxesTotal'):
                v = self.rounding_taxes(k, v, rounding)
                xml_values[k] = v
                print('\n\n********IMPUESTOS CABECERA***********\n\n')
            elif k in ('InvoiceLines',):
                for lines_k, lines_v in v.items():
                    for line_k, line_v in lines_v.items():
                        if line_k in ('PriceAmount', 'LineExtensionAmount', 'MultiplierFactorNumeric', 'AllowanceChargeAmount', 'AllowanceChargeBaseAmount'):
                            line_v = '{:.2f}'.format(round(float(line_v), rounding))
                            xml_values[k][lines_k][line_k] = line_v
                            print('\n\n**********LINEAS*********\n\n')
                        elif line_k in ('TaxesTotal', 'WithholdingTaxesTotal'):
                            line_v = self.rounding_taxes(line_k, line_v, rounding)
                            xml_values[k][lines_k][line_k] = line_v
                            print('\n\n********IMPUESTOS LINEAS***********\n\n')
        return xml_values
    
    def rounding_taxes(self, k_taxes, v_taxes, rounding=2):
        self.ensure_one()
        for tk, tv in v_taxes.items():
            v_taxes[tk]['total'] = round(tv['total'], rounding)
            for pk, pv in tv['taxes'].items():
                v_taxes[tk]['taxes'][pk]['base'] = round(pv['base'], rounding)
                v_taxes[tk]['taxes'][pk]['amount'] = round(pv['amount'], rounding)
            print(tk, tv)
        return v_taxes
