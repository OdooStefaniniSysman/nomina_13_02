...
class MultiInvoicePayment(models.TransientModel):
	...
	is_customer = fields.Boolean(string="Es un Cliente")
	...
	def default_get(self, fields):
		...
		if any(invoice.state != 'open' for invoice in invoices):
			raise UserError(_("Solo puede registrar pagos de facturas en estado"
							  " Abierto"))
		...
		if any(inv.currency_id != invoices[0].currency_id for inv in invoices):
			raise UserError(_("Para pagar multiples facturas a la vez, ellas"
		...
		if MAP_INVOICE_TYPE_PARTNER_TYPE[invoices[0].type] == 'customer':
			for inv in invoices:
				inv_list.append((0,0,{
					...
					#'amount_to_pay' : 0.0,
                    'amount_to_pay' : inv.residual or 0.0,
					}))
	...
	def register_multi_payment(self):
		if self.customer_invoice_ids:
			...
			raise UserError(_("Algo salio mal... Consulte con el administrador del sistema \n"))
...
class InvoiceLines(models.TransientModel):
	...
	total_amount = fields.Float("Valor Facturado", required=True)
	amount_to_pay = fields.Float(string='Valor Pagado')
	payment_diff = fields.Float(string='Valor Adeudado',store=True,readonly=True)
...
class InvoiceLines(models.TransientModel):
	...
	payment_method_id = fields.Many2one('account.payment.method',string='Tipo de Pago')
	total_amount = fields.Float("Valor Facturado", required=True)
	payment_diff = fields.Float(string='Valor Pagado',store=True,readonly=True)
	amount_to_pay = fields.Float(string='Valor Adeudado')
