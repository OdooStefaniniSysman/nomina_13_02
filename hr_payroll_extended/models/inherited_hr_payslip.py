...
class HrPayslip(models.Model):
    ...
    def _onchange_employee(self):
        ...
        values = {
            'struct_id': contract_id and contract_id.struct_id.id or None,
            ...
            'voluntary_contribution_id': self.employee_id.voluntary_contribution_id.id,
            'voluntary_contribution2_id': self.employee_id.voluntary_contribution2_id.id,
    ...
    def create(self, vals):
        if vals.get('contract_id'):
            ...
            if contract and str(contract.date_end) == str(vals.get('date_to')):
                contract_completion_id = self.env['hr.contract.completion'].search(
                    [('employee_id', '=', vals.get('employee_id')),
                     ('date', '=', contract.date_end)])
                if not contract_completion_id:
                    raise ValidationError(
                        _("You don't can create this payslip why date end contract is contain in payslip, "
                          "You must be review to generate extension contract or generate contract completion"))
        ...
        vals.update({
            ...
            'voluntary_contribution_id': employee_id.voluntary_contribution_id.id,
            'voluntary_contribution2_id': employee_id.voluntary_contribution2_id.id,
    ...
    @api.model
    def _get_worked_day_lines(self, contracts=False):
        ...
        for contract in contracts.filtered(
            ...
            if ct_id and ct_id.date.month != self.date_from.month and \
                ...
                for attendance in res:
                    ...
                    if entry_type:
                        attendance['work_entry_type_id'] = entry_type.id
                    else:
                        raise ValidationError(_("Code " + code + " doesn't have correct Work Entry Type"))
        ...
        if date_from and date_to and \
            ...
            for leave_rec in self.env.cr.fetchall():
                ...
                if not leave.holiday_status_id.no_count_in_payroll:
                    ...
                    if leave.holiday_status_id.vacation_pay_31 and \
                        ...
                        if leave.request_date_to >= leave.request_date_from.replace(day=31) and \
                                date_to == leave.request_date_from.replace(day=31):  # and not leave.after_close:
                            day_additional = 1
                    if leave.request_date_to.month == \
                            leave.request_date_from.month:  # and not leave.after_close:
                        if leave.holiday_status_id.request_unit == 'hour':
                            leave_days += (leave.number_of_days)
                        else:
                            if date_from > leave.request_date_from:
                    ...
                    if leave.request_date_to.month != \
                        ...
                        if leave.request_date_from > date_from and \
                            ...
                            leave_days += ((leave.request_date_to -
                                            leave.request_date_from).days + 1)
                        if leave.request_date_from > date_from and \
                                leave.request_date_to > date_to:  # and not leave.after_close:
                            leave_days += ((date_to -
            ...
            for r in res:
                if r['code'] == 'WORK100':
                    r['number_of_days'] = total_days - leave_days + day_additional
        ...
        for leave_rec in self.env.cr.fetchall():
            ...
            if not leave.holiday_status_id.no_count_in_payroll:
                ...
                if leave.holiday_status_id.vacation_pay_31 and \
                        context_start.month in [1, 3, 5, 7, 8, 10, 12]:
                    if leave.request_date_to >= leave.request_date_from.replace(day=31) and \
                            date_to == leave.request_date_from.replace(day=31) and \
                            date_to < leave.request_date_to:  # and not leave.after_close:
                        day_additional = 0
                if leave.request_date_to.month == \
                        leave.request_date_from.month:  # and not leave.after_close:
                    if leave.holiday_status_id.request_unit == 'hour':
                ...
                if not leave_line.get(leave.holiday_status_id.name, ''):
                    leave_line.update(
                        {leave.holiday_status_id.name: leave_days + day_additional})
                else:
                    total_days = leave_line.get(leave.holiday_status_id.name, '')
                    leave_line.update({
        ...
        if leave_yes_total > 0:
            ...
            work100_val.update(
                {'number_of_days': leave_yes_total + work100_val.get(
                    'number_of_days', '')})
        if is_vacation_pay_31 and next((item for item in res if item["code"] == "WORK100"), False):
            work100_val = next((item for item in res if item[
                "code"] == "WORK100"), False)
            work100_val.update(
                {'number_of_days': 1.0 + work100_val.get(
                    'number_of_days', '')})
        # for val in res:
        #    p_id = {'payslip_id' : self.id}
        #    val.update(p_id)
        #    self.env['hr.payslip.worked_days'].create(val)
